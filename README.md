...
При недоступности (rate-limit, блокировка IP) автоматически
генерирует реалистичные демо-данные — это позволяет демонстрировать
дашборды без доступа к API.

**Для реального парсинга** запускайте приложение с вашего IP-адреса
(не облачного сервера), которому WB не выставляет rate-limit.
При необходимости добавьте прокси в `_wb_session` в `app.py`.
